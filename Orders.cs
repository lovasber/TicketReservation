...
namespace TicketReservation
{
    public partial class Orders : Form
    {
        ...
        private void setSum()
        {
            ...
            cmd = new SQLiteCommand("SELECT sum(Ticket.Price) FROM User, User_Order, Ticket, Path WHERE User.name = User_Order.User_id AND Ticket.Id = User_Order.Ticket_id AND Path.Id = Ticket.Path_Id AND Path.Leaving_from like '"+leaving+"' AND Path.Destination like '"+destination+"';",
                db.GetConnection()
            );
            sda.Fill(dt);
